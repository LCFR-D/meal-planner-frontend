...
export default function App() {
  ...
  // derive tags from the currently loaded recipes (API filters server-side)
  const allTags = useMemo(
    () =>
      Array.from(
        new Set(recipes.flatMap(r => (r.tags || r.cuisines || []).map(t => t.trim()).filter(Boolean)))
      ).sort(),
    [recipes]
  );

  // fetch recipes + plans whenever filters/paging or week range change
  useEffect(() => {
    let ignore = false;
    (async () => {
      setLoading(true);
  ...
  const onAssign = async (dateISO, slot, recipe) => {
    try {
      ...
      setToast({
        ...
        desc: `${recipe.name || recipe.title} on ${dayjs(dateISO).format("ddd D MMM")} (${slot})`
      });
    } catch (e) {
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-4 md:pt-6 pb-6">
        <section className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            {/* Cards are draggable; no onAssign button needed */}
            <RecipesGrid loading={loading} error={error} recipes={recipes} />
          </div>
        </section>
      </main>
